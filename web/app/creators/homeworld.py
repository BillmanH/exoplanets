...
def build_species(data):
    species = {}
    for attr in ['population_conformity',
                'population_literacy',
...
def get_pop_name(factions,faction_no):
    # the pop name is the faction name plus an extra syllable.
    df = pd.DataFrame(factions)
    name = df[df['id']==2]['name'].values[0] + " " + language.make_word(1)
    return name
...
def build_people(data):
    ...
    pops['name'] = pops['faction_no'].apply(lambda x: get_pop_name(factions,x))
